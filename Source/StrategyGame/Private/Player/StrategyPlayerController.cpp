...
void AStrategyPlayerController::BeginPlay() {
	...
	UGameplayStatics::GetAllActorsOfClass(GetWorld(), AStrategyBuilding_Brewery::StaticClass(), FoundActors);

	for (AActor* FoundActor : FoundActors)
	{
		AStrategyBuilding_Brewery* TypedActor = Cast<AStrategyBuilding_Brewery>(FoundActor);
		uint8 Team = TypedActor->GetTeamNum();
		if (Team == EStrategyTeam::Player)
		{
			AlliedBrewery = TypedActor;
		}
	}
}

void AStrategyPlayerController::MoveForward(float Val)
{
	if (PossessedMinion.IsValid() && PossessedMinion.Get() != nullptr)
	{
		PossessedMinion->MoveForward(Val);
	}
}

void AStrategyPlayerController::MoveRight(float Val)
{
	if (PossessedMinion.IsValid() && PossessedMinion.Get() != nullptr)
	{
		PossessedMinion->MoveRight(Val);
	}
}
